...
parser.add_argument('--batch_size', type=int, default=16, help='batch size') #Fixed
...
def local_update(device, model, train_set):
...
#############################################################################################
    ...
    cnn.to(args.device_local)
    target_accuracy=[] 
    total_time=0;
    for epoch in range(args.global_epoch):
        start_time = time.time()
        cnn = local_update(args.device_local, cnn, target_train)
        auc, acc, ka = evaluate_model(target_test, cnn, args.device_local)
